...
namespace net::rpc::routing_neighbor_send_hello {
    template <frame::IFrameService FrameService>
    class Executor {
        Request<FrameService> request_;
        link::AsyncAddressParser peer_parser_;
        routing::AsyncCostParser cost_parser_;
      ...
      public:
        ...
        nb::Poll<void> execute(
            ...
            if (!response_.has_value()) {
                ...
                POLL_UNWRAP_OR_RETURN(request_.body().read(cost_parser_));

                const auto &peer = peer_parser_.result();
                const auto cost = cost_parser_.result();
                POLL_UNWRAP_OR_RETURN(routing_service.request_hello(peer, cost));
