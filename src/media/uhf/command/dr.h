...
namespace media::uhf {
    ...
    class DRExecutor {
      ...
      public:
        DRExecutor(Serial &&serial, nb::Promise<ResponseReader<Serial>> &&body)
            : serial_{memory::Owned{etl::move(serial)}},
              body_{etl::move(body)} {}

        nb::Poll<nb::Empty> poll() {
            if (!length_.is_writer_closed()) {
                nb::stream::pipe_writers(serial_.get(), prefix_, length_);
                if (!length_.is_writer_closed()) {
                    return nb::pending;
                }
                ...
                auto serial = etl::move(serial_.try_create_pair().value());
                ResponseReader<Serial> reader{etl::move(serial), length};
            ...
            if (!suffix_.is_writer_closed()) {
                nb::stream::pipe_writers(serial_.get(), suffix_);
                if (!suffix_.is_writer_closed()) {
                    return nb::pending;
                }
            }
